"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var core_1 = require("@angular/core");
var router_1 = require("@angular/router");
var table_service_1 = require("./table.service");
var TablesComponent = (function () {
    function TablesComponent(tableService, router) {
        this.tableService = tableService;
        this.router = router;
        this.title = 'RPGtables';
    }
    TablesComponent.prototype.ngOnInit = function () {
        this.getTables();
    };
    TablesComponent.prototype.onSelect = function (table) {
        this.selectedTable = table;
    };
    TablesComponent.prototype.getTables = function () {
        var _this = this;
        this.tableService.getTablesSlowly().then(function (tables) { return _this.tables = tables; });
    };
    TablesComponent.prototype.goToTable = function () {
        this.router.navigate(['/table', this.selectedTable.id]);
    };
    return TablesComponent;
}());
TablesComponent = __decorate([
    core_1.Component({
        selector: 'my-tables',
        templateUrl: './tables.component.html',
        styleUrls: ['./tables.component.css']
    }),
    __metadata("design:paramtypes", [table_service_1.TableService,
        router_1.Router])
], TablesComponent);
exports.TablesComponent = TablesComponent;
//# sourceMappingURL=tables.component.js.map